...
impl Schema {
    ...
    fn process_assignment(&mut self, ass: &crate::ast::Assignment, metadata: &[String], kind: Option<&str>) {
            ...
            if line.starts_with("###") {
                ...
                if let Some(eq) = line.find('=') {
                    ...
                    if parts.len() == 2 {
                        ...
                        let max = if parts[1] == "inf" { None } else { parts[1].parse().ok() };
                ...
                if let Some(eq) = line.find('=') {
                    match line[eq + 1..].trim().to_lowercase().as_str() {
                        ...
                        "information" | "info" => current_severity = Some(DiagnosticSeverity::Information),
        ...
        if ass.key.starts_with("alias[") && ass.key.contains(':') && ass.key.ends_with(']') {
            ...
            if let Some(colon) = inner.find(':') {
                ...
                match alias_kind {
                    "trigger" => { self.triggers.insert(key.to_string(), rule); }
                    "effect" => { self.effects.insert(key.to_string(), rule); }
                    _ => {}
                }
            }
            ...
            if k == "trigger" { self.triggers.insert(ass.key.clone(), rule); }
            else if k == "effect" { self.effects.insert(ass.key.clone(), rule); }
        }
    }
    ...
    fn parse_value_type(&self, val: &crate::ast::Value) -> ValueType {
        match val {
            crate::ast::Value::String(s) => {
                ...
                if s_lower == "bool" { ValueType::Bool }
                else if s_lower == "int" { ValueType::Int }
                else if s_lower == "float" { ValueType::Float }
                else if s_lower == "string" { ValueType::String }
