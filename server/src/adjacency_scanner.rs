    ...
    F: Fn(&std::path::Path) -> bool,
{
    ...
    for root in roots {
        ...
        if adj_csv_path.exists() && !filter(&adj_csv_path) && let Ok(content) = fs::read_to_string(&adj_csv_path) {
            for (line_idx, line) in content.lines().enumerate() {
                ...
                if parts.len() >= 9 && let (Ok(start_prov), Ok(end_prov)) =
        ...
        if rules_path.exists() && !filter(&rules_path) && let Ok(content) = fs::read_to_string(&rules_path) {
            let (script, _) = parser::parse_script(&content);
            for entry in script.entries {
                if let ast::Entry::Assignment(ass) = entry && ass.key.to_lowercase() == "adjacency_rule" && let ast::Value::Block(rule_entries) = &ass.value.value {
                    let mut name = None;
                                ...
                                "required_provinces" => {
                                    if let ast::Value::Block(prov_entries) = &r_ass.value.value {
                                        for p_entry in prov_entries {
                                            if let ast::Entry::Value(p_val) = p_entry && let ast::Value::Number(n) = &p_val.value {
