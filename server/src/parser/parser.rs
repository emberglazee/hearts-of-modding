...
fn to_range(span: Span) -> crate::parser::ast::Range {
...
fn identifier(input: Span) -> IResult<Span, (String, crate::parser::ast::Range)> {
...
fn quoted_string(input: Span) -> IResult<Span, (String, crate::parser::ast::Range)> {
    ...
    loop {
        ...
        if c == '"' {
            let range = crate::parser::ast::Range {
...
fn number(input: Span) -> IResult<Span, (f64, crate::parser::ast::Range)> {
...
fn boolean(input: Span) -> IResult<Span, (bool, crate::parser::ast::Range)> {
...
fn comment(input: Span) -> IResult<Span, (String, crate::parser::ast::Range)> {
    let (input, s) =
        recognize(preceded(char('#'), take_while(|c| c != '\n' && c != '\r'))).parse(input)?;
    Ok((input, (s.fragment()[1..].to_string(), to_range(s))))
}

fn operator(input: Span) -> IResult<Span, (Operator, crate::parser::ast::Range)> {
    alt((
...
fn parse_block(input: Span) -> IResult<Span, (Vec<Entry>, crate::parser::ast::Range)> {
...
fn to_error_range(span: Span) -> crate::parser::ast::Range {
    let start_line = span.location_line() - 1;
    let start_col = span.get_column() as u32 - 1;
    let fragment = span.fragment();
