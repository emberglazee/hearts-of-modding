...
/// Closed set of entity kinds. The map/logistics families
/// (`SupplyNode`, `Railway`, `MapBuilding`, `UnitStack`, `WeatherPosition`,
/// `Adjacency`) have no lookup path that constructs them yet — goto-definition
/// resolves script entities only — but they document the mapping.
#[allow(dead_code)]
pub enum EntityKind {
    ScriptedTrigger,
    ScriptedEffect,
...
pub struct EntityLocation {
    /// Constructed at every result site; no live reader yet.
    #[allow(dead_code)]
    pub kind: EntityKind,
    pub range: ast::Range,
